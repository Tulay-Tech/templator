...
// Import the queryClient with global error handling
  ...
  beforeLoad: async ({ location, context }) => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        // Redirect away from org-select if active org is already set
        if (hasOrgs && activeOrgSet && isOrgSelectRoute) {
          throw redirect({ to: "/" });
        }
      }
    } catch (error) {
      // If error is a 401, the global handler will have already logged out
      ...
      <TooltipProvider>
        {isAuthRoute ? (
          ...
          <SidebarProvider>
            ...
            />
            <SidebarInset>
              <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12 justify-between px-2">
